...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        #print("消除順序：", eliminationOrder)
  
    
        for elim in eliminationOrder:
            currentFactorsNotToJoin, joinedFactor = joinFactorsByVariable(currentFactorsNotToJoin, elim)
            if len(joinedFactor.unconditionedVariables()) > 1:
                currentFactorsNotToJoin.append(eliminate(joinedFactor, elim))
        #print("後得到的新因子：", joinedFactor)
        #print("未聯合的因子列表：", currentFactorsNotToJoin)
        
        return normalize(joinFactors(currentFactorsNotToJoin))
        raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        for k, val in self.items():
            cumulativeP += val / total
            if   target<  cumulativeP:
                return k
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPos in self.allPositions:
            newBelief [ghostPos] = self.getObservationProb(observation, gameState.getPacmanPosition(), ghostPos, self.getJailPosition()) *self.beliefs[ghostPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        Len = len(legalPositions)
        PositionPart = numParticles // Len
        remainingPart = numParticles%Len
        for position in legalPositions:
            self.particles.extend([position] * PositionPart)
    
        self.particles.extend(legalPositions[:remainingPart])
    ...
    def elapseTime(self, gameState):
        ...
        newParts = []
        for particle in self.particles:
            newParts.append(self.getPositionDistribution(gameState, particle).sample())
