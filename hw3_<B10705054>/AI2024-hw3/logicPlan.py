...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    disjunctions = [(~pair[0]|~pair[1]) for pair in combinations]
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    successor = PropSymbolExpr(pacman_str, x, y, time=now)
   
    current= disjoin(possible_causes)
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    model_pacman_at_1 = findModel(conjoin([conjoin(KB), PropSymbolExpr(pacman_str, x1, y1, time=1)]))
    model_no_pacman_at_1 = findModel(conjoin([conjoin(KB), ~PropSymbolExpr(pacman_str, x1, y1, time = 1)]))
   

    "*** END YOUR CODE HERE ***"

    return model_pacman_at_1, model_no_pacman_at_1
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        food_negations = [~PropSymbolExpr(food_str, x, y, time=t) for x, y in food]
        goal = conjoin(food_negations)
        ...
        for x, y in food:
            ...
            KB.append(( PacmanNow | ~FoodNow) % ~FoodNext)
